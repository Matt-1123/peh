...
import { getTravelAction } from "../graphql/queries";
import { updateTravelAction } from "../graphql/mutations";
const client = generateClient();
export default function TravelActionUpdateForm(props) {
  const {
    id: idProp,
    travelAction: travelActionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = travelActionRecord
      ? { ...initialValues, ...travelActionRecord }
      : initialValues;
    setDate(cleanValues.date);
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setUsedTravelType(cleanValues.usedTravelType);
    setUsedDistance(cleanValues.usedDistance);
    setAvoidedTravelType(cleanValues.avoidedTravelType);
    setAvoidedDistance(cleanValues.avoidedDistance);
    setCarbonPrevented(cleanValues.carbonPrevented);
    setUser(cleanValues.user);
    setErrors({});
  };
  const [travelActionRecord, setTravelActionRecord] = React.useState(
    travelActionModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTravelAction.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTravelAction
        : travelActionModelProp;
      setTravelActionRecord(record);
    };
    queryData();
  }, [idProp, travelActionModelProp]);
  React.useEffect(resetStateValues, [travelActionRecord]);
  const validations = {
    date: [{ type: "Required" }],
    title: [{ type: "Required" }],
    description: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTravelAction.replaceAll("__typename", ""),
            variables: {
              input: {
                id: travelActionRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TravelActionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || travelActionModelProp)}
